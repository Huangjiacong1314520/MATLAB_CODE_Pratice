...
save('DATA\Generate_Data_NN_Input.mat', 'y');                              %用于生成MATLAB神经网络工具箱训练数据，作为神经网络的输入
% save('E:\A_Motion_Control_Team\MATLAB_CODE_Pratice\MATLAB_NN_Toolbox\Generate_Data\DATA\Generate_Data_NN_Outputput.mat', 'r');    %用于生成MATLAB神经网络工具箱训练数据，作为神经网络的输出
save('DATA\Generate_Data_NN_Outputput.mat','Ufb');                   %用于生成MATLAB神经网络工具箱训练数据，作为神经网络的输出

%然后把生成的数据粘贴到另一个文件夹（神经网络训练文件夹）
save('E:\A_Motion_Control_Team\MATLAB_CODE_Pratice\MATLAB_NN_Toolbox\2NN_Train\Generate_Data_NN_Input.mat', 'y');                              %用于生成MATLAB神经网络工具箱训练数据，作为神经网络的输入
% save('E:\A_Motion_Control_Team\MATLAB_CODE_Pratice\MATLAB_NN_Toolbox\Generate_Data\DATA\Generate_Data_NN_Outputput.mat', 'r');    %用于生成MATLAB神经网络工具箱训练数据，作为神经网络的输出
save('E:\A_Motion_Control_Team\MATLAB_CODE_Pratice\MATLAB_NN_Toolbox\2NN_Train\Generate_Data_NN_Outputput.mat','Ufb');                   %用于生成MATLAB神经网络工具箱训练数据，作为神经网络的输出


%想要绘制r y error0的图像，确保生成的数据正确——聪
% 假设 t 是时间向量
...
% 绘制参考轨迹位置 r
subplot(2, 1, 1); % 创建一个 3 行 1 列的子图，并选择第一个
...
plot(t, y, 'r', 'LineWidth', 1.5); % 使用红色线绘制速度
xlabel('时间 (s)');
ylabel('速度 (单位)');
title('速度随时间的变化');
grid on;
...
% 
figure(2);
plot(t,err0,'k','linewidth',1.2);                                          %绘制一下error0的图像，确保生成的数据正确
% hold on;
